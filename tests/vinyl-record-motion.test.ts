...
import type { PlayerTrack } from '../src/context/PlayerContext';
import {
...
} from '../src/components/vinylRecordMotion';
...
describe('getTonearmRotation', () => {
  ...
  it('tracks inward with playback and clamps progress', () => {
...
describe('getControlledTurntableTrack', () => {
  it('keeps the featured song on the deck while another library song is active', () => {
  ...
  it('animates only when the song loaded on the deck is playing', () => {
    const state = getControlledTurntableTrack({
  ...
  it('falls back to the current song when no featured song is available', () => {
    const state = getControlledTurntableTrack({
      currentTrack: libraryTrack,
      featuredTrack: null,
      isPlaying: true,
